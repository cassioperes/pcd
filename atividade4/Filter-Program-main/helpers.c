...
{
    int i;
    RGBTRIPLE **tmp = (RGBTRIPLE**)malloc(height * sizeof(RGBTRIPLE*));
    #pragma omp parallel for \
     shared(height, tmp)
    for(i = 0; i < height; i++){
        tmp[i] = (RGBTRIPLE*)malloc(width * sizeof(RGBTRIPLE));
    }

    #pragma omp parallel for \
     shared(height, width, image, tmp)
    for (int row = 0; row < height; row ++)
    {
        for (int col = 0; col < width; col ++)
        {
            ...
            int xaxis[] = {row - 4, row - 3, row - 2, row - 1, row, row + 1, row + 2, row + 3, row + 4};
            int yaxis[] = {col - 4, col - 3, col - 2, col - 1, col, col + 1, col + 2, col + 3, col + 4};
