...
public class Grid
{
	...
	public void makeNewGeneration() throws InterruptedException
	{
	    ...
	    for(gen = 1; gen <= num_gen; gen++)
	    {
	    	for(i=0; i<THREADS; i++)
	    	{
				nG[i] = new newGeneration(i*lines,(i+1)*lines,this, newGrid);
				t[i] = new Thread(nG[i]);
				t[i].start();
			}
